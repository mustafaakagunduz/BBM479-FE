...
// SurveyBuilder component'inin başındaki state tanımlamalarına:
    ...
    const validateQuestions = () => {
        // Check if number of questions matches number of selected skills
        ...
        },
        {
            ...
            content: (
                <div className="space-y-6">
                    {/* Warning message about required questions */}
                    {selectedSkills.length > 0 && (
                        <div className="mb-4 p-4 bg-purple-50 rounded-lg">
                            <p className="text-purple-700">
                                You need to create {selectedSkills.length} question{selectedSkills.length !== 1 ? 's' : ''}
                    ...
                    {/* Add Question Button - Updated version */}
                    <button
                        onClick={() => {
                            if (questions.length < selectedSkills.length) {
                                setQuestions([...questions, createNewQuestion()]);
                            } else {
                                toast.error(`You can only add ${selectedSkills.length} questions (one per skill)`);
                            }
                        }}
                        disabled={questions.length >= selectedSkills.length}
                        className={`w-full py-4 rounded-lg bg-gradient-to-r from-purple-500 to-pink-500 text-white font-medium 
